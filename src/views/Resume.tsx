...
const DateRange = ({
  ...
  return (
    <p className="mono-label shrink-0 text-subtle print:text-[8.5pt] print:tracking-normal print:normal-case">
      <time dateTime={startISO}>{t(startDate)}</time>
      {" – "}
      {endISO ? <time dateTime={endISO}>{t(endDate)}</time> : t(endDate)}
    </p>
  );
};
    ...
    <article className="mx-auto max-w-3xl px-5 py-12 print:max-w-none print:px-0 print:py-0">
      <div className="mb-10 flex flex-wrap items-center gap-3 print:hidden">
          <button
            ...
            {pending ? t(ui.generating) : t(ui.downloadPdf)}
          </button>
          <button
            type="button"
          ...
          {visibleExperiences.map((job) => (
            <div key={job.company} className="print-break-avoid">
              ...
              <p className="mono-label mt-1 text-subtle print:mt-0 print:text-[8.5pt] print:tracking-normal print:normal-case">
                {t(job.location)} · {t(job.employmentType)}
              </p>
              ...
              <ul className="mt-4 space-y-2 print:mt-1.5 print:space-y-0.5">
                {t(job.highlights).map((highlight) => (
                  <li
                    key={highlight}
                    className="relative ps-5 text-sm leading-relaxed print:ps-4 print:text-[9.5pt]">
                    <span
                      aria-hidden="true"
                      className="absolute start-0 top-[0.55em] size-1.5 rounded-full bg-accent
              ...
              <p className="mt-4 text-sm text-subtle print:mt-1.5 print:text-[9pt]">
                <span className="font-medium">{t(ui.technologies)}:</span>{" "}
                {job.stack.join(", ")}
              </p>
            </div>
          ))}
      ...
      <Section title={t(ui.resumeSkills)}>
      ...
      <Section title={t(ui.resumeProjects)}>
            ...
            <li
              ...
              <div>
                <h3 className="text-base font-semibold print:text-[10pt]">
                  {t(school.degree)}, {t(school.field)}
                </h3>
                <p className="text-sm text-muted print:text-[9.5pt]">
                  {t(school.institution)}
                </p>
              </div>
              <DateRange
