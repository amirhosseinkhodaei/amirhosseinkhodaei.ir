...
const featured = visibleProjects.filter((project) => project.highlights);
const rest = visibleProjects.filter((project) => !project.highlights);
    ...
    <a
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {project.role && (
            <p className="mono-label mt-1.5 text-accent">{t(project.role)}</p>
          )}
        </div>
        <Icon
          name="arrow-up-right"
      ...
      {project.highlights && (
        <ul className="mt-5 grid gap-2 sm:grid-cols-2">
          {t(project.highlights).map((highlight) => (
            <li
              key={highlight}
              className="relative ps-5 text-sm leading-relaxed text-muted">
              <span
                aria-hidden="true"
